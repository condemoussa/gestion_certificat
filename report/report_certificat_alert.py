...
class RapportCertificatsAlert(models.AbstractModel):
    _name = 'report.gestion_certificat.wizard_rapport_alert_certificat'
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        sheet = workbook.add_worksheet('RAPPORT DES CERTIFICATS EN ALERT')
        ...
        sheet.merge_range('D3:H3', "LA LISTE DES CERTFICATS EN ALERT", header_format)
        ...
        sheet.write(row, col + 13, 'COMMENTAIRE', header_format11)
        ...
        for line in range(len(data_payroll)):
            if data_payroll[line]['status'] =='red':
                ...
                background = workbook.add_format(
                    {'bg_color': 'blue', 'align': 'center', 'valign': 'vcenter', 'size': 8,
                     'color': 'white','bold': 'TRUE'})
            ...
            date_object = datetime.strptime(date_str, '%Y-%m-%d')
            formatted_date = date_object.strftime('%d/%m/%Y')
            sheet.write(row, col + 9,formatted_date, background)
            ...
            sheet.write(row, col + 13, data_payroll[line]['comment'] if data_payroll[line]['comment'] else "",   background)
